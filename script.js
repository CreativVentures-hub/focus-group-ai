...
document.addEventListener('DOMContentLoaded', function() {
    ...
    debugButton.addEventListener('click', function() {
        ...
        console.log('Gender modal:', document.getElementById('genderModal'));
        console.log('Open gender button:', document.getElementById('openGenderModal'));
        console.log('Slider:', document.getElementById('numberOfParticipants'));
        console.log('Slider value:', document.getElementById('numberOfParticipantsValue'));
        
        // Test opening gender modal
        const genderModal = document.getElementById('genderModal');
        const openGenderBtn = document.getElementById('openGenderModal');
        
        if (genderModal && openGenderBtn) {
            console.log('Testing gender modal...');
            genderModal.style.display = 'flex';
            setTimeout(() => {
                genderModal.style.display = 'none';
                console.log('Gender modal test complete');
            }, 2000);
        } else {
            console.error('Gender modal elements not found');
        }
    });
    
    document.body.appendChild(debugButton);
    
    webhookTestButton.addEventListener('click', async function() {
        console.log('=== TESTING WEBHOOK ===');
        
        if (!CONFIG.WEBHOOK_URL) {
            alert('ℹ️ Demo Mode: Webhook URL is not configured. The form will work in demo mode.\n\nTo use a real webhook, set CONFIG.WEBHOOK_URL in config.js');
            return;
        }
        
        try {
            const testData = {
                session_type: 'Market Research',
            ...
            if (contentType && contentType.includes('text/plain') && contentDisposition && contentDisposition.includes('attachment')) {
                ...
                const filename = getFilenameFromDisposition(contentDisposition) || `test-focus-group-${Date.now()}.txt`;
                
                console.log('File download received:', filename);
                alert(`✅ SUCCESS: File download received!\nFilename: ${filename}\nSize: ${blob.size} bytes\n\nCheck console for details.`);
                
                // Optionally show the file download screen for testing
                if (confirm('Would you like to see the file download interface?')) {
                    showFileDownloadScreen({
                        filename: filename,
                        blob: blob,
                        sessionName: testData.session_name,
                        categories: testData.categories,
                        totalCount: testData.number_of_participants
                    });
                }
            } else {
                ...
                try {
                    ...
                    if (response.ok) {
                        alert('✅ SUCCESS: Webhook is working! Check console for details.');
                    } else {
                        alert(`❌ ERROR: Webhook error: ${response.status} - ${result.message || 'Unknown error'}`);
                    }
                } catch (jsonError) {
            ...
            alert(`❌ ERROR: Webhook test failed: ${error.message}`);
        }
    });
    
    document.body.appendChild(webhookTestButton);
    
    // Add a simple connectivity test button
    const connectivityTestButton = document.createElement('button');
    connectivityTestButton.textContent = 'Test Connectivity';
    connectivityTestButton.style.position = 'fixed';
    connectivityTestButton.style.top = '170px';
    connectivityTestButton.style.right = '10px';
    connectivityTestButton.style.zIndex = '9999';
    connectivityTestButton.style.padding = '10px';
    connectivityTestButton.style.background = 'teal';
    connectivityTestButton.style.color = 'white';
    connectivityTestButton.style.border = 'none';
    connectivityTestButton.style.borderRadius = '5px';
    connectivityTestButton.style.cursor = 'pointer';
    
    connectivityTestButton.addEventListener('click', async function() {
        ...
        try {
            ...
            const testResponse = await fetch(webhookUrl, {
                ...
                mode: 'cors',
                credentials: 'omit',
                signal: AbortSignal.timeout(15000) // 15 seconds for connectivity test
            });
            
            console.log('GET response status:', testResponse.status);
            alert(`✅ Basic connectivity works!\nStatus: ${testResponse.status}\n\nNote: GET requests might not work with webhooks, but this confirms the server is reachable.`);
            
        } catch (error) {
            console.error('Connectivity test failed:', error);
            
            if (error.name === 'AbortError') {
                alert('❌ Timeout: Server did not respond within 10 seconds');
            } else if (error.message.includes('Failed to fetch')) {
                alert('❌ CORS Error: Server is reachable but blocking cross-origin requests\n\nThis is the most common issue. The n8n server needs CORS headers.');
            } else {
    ...
    document.body.appendChild(connectivityTestButton);
    
    // Add a CORS proxy toggle button
    const corsProxyButton = document.createElement('button');
    corsProxyButton.textContent = 'CORS Proxy: OFF';
    corsProxyButton.style.position = 'fixed';
    corsProxyButton.style.top = '210px';
    corsProxyButton.style.right = '10px';
    corsProxyButton.style.zIndex = '9999';
    corsProxyButton.style.padding = '10px';
    corsProxyButton.style.background = 'darkred';
    corsProxyButton.style.color = 'white';
    corsProxyButton.style.border = 'none';
    corsProxyButton.style.borderRadius = '5px';
    corsProxyButton.style.cursor = 'pointer';
    
    corsProxyButton.addEventListener('click', function() {
        CONFIG.USE_CORS_PROXY = !CONFIG.USE_CORS_PROXY;
        corsProxyButton.textContent = `CORS Proxy: ${CONFIG.USE_CORS_PROXY ? 'ON' : 'OFF'}`;
        corsProxyButton.style.background = CONFIG.USE_CORS_PROXY ? 'darkgreen' : 'darkred';
        console.log(`CORS proxy ${CONFIG.USE_CORS_PROXY ? 'enabled' : 'disabled'}`);
        alert(`CORS Proxy ${CONFIG.USE_CORS_PROXY ? 'ENABLED' : 'DISABLED'}\n\nThis will use a proxy to bypass CORS restrictions.\n\nNote: This is for development only and may have rate limits.`);
    });
    
    document.body.appendChild(corsProxyButton);
    
    // Add a webhook URL test button
    const webhookUrlTestButton = document.createElement('button');
    webhookUrlTestButton.textContent = 'Test URL';
    webhookUrlTestButton.style.position = 'fixed';
    webhookUrlTestButton.style.top = '250px';
    webhookUrlTestButton.style.right = '10px';
    webhookUrlTestButton.style.zIndex = '9999';
    webhookUrlTestButton.style.padding = '10px';
    webhookUrlTestButton.style.background = 'orange';
    webhookUrlTestButton.style.color = 'white';
    webhookUrlTestButton.style.border = 'none';
    webhookUrlTestButton.style.borderRadius = '5px';
    webhookUrlTestButton.style.cursor = 'pointer';
    
    webhookUrlTestButton.addEventListener('click', async function() {
        ...
        try {
            // Test the URL directly without any proxy
            console.log('Testing URL:', CONFIG.WEBHOOK_URL);
            ...
            if (response.type === 'opaque') {
                alert('✅ URL is reachable (CORS blocked, but server responds)\n\nThis confirms the webhook URL is valid and the server is running.\n\nThe issue is CORS - the server needs to allow requests from your domain.');
            } else {
                alert(`✅ URL is reachable!\nStatus: ${response.status}\nType: ${response.type}\n\nThis means the webhook URL is valid and accessible.`);
            }
            ...
            if (error.name === 'AbortError') {
                ...
                alert('❌ Connection failed: Unable to reach the webhook URL\n\nThis could mean:\n• The URL is incorrect\n• The server is down\n• Network connectivity issues');
            } else {
